...
const Layout = ({ children, hideNav = false }: LayoutProps) => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background font-['Outfit']">
      ...
      {!hideNav && (
        <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 shadow-lg safe-area-inset-bottom">
          <div className="flex justify-around items-center h-16 px-4">
            <NavLink to="/" active={location.pathname === '/'} icon={<Home />} label="Accueil" />
            <NavLink 
              to="/watch-ad" 
              active={location.pathname === '/watch-ad'} 
              icon={<Play className="text-brand-purple" />} 
              label="Regarder" 
              className={cn(
                "relative -top-5 bg-white dark:bg-gray-900 rounded-full p-4 border-4 border-brand-purple shadow-lg",
